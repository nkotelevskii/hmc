...
class HMC_our(nn.Module):
    def __init__(self, args):
        ...
        self.N = args.N  # num leapfrogs
        self.alpha_logit = nn.Parameter(
            torch.tensor(np.log(args.alpha) - np.log(1. - args.alpha), device=self.device), requires_grad=True)
        self.use_barker = args.use_barker  # If false, we are using standard MH ration, otherwise Barker ratio
        ...
        self.naf = None
        self.learnable_accept = args.learnable_accept
    ...
    def make_transition(self, q_old, p_old, target_distr, k=None, x=None, accept_func=None, h=None):
        ...
        if self.learnable_accept:
            log_t = accept_func(q_upd, q_old, h).view(-1) + self.std_normal.log_prob(p_upd).sum(
                1) - self.std_normal.log_prob(p_ref).sum(1)
